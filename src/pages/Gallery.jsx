...
import { GALLERY_PROJECT_POSTS } from "../data/galleryProjects";

// Personal: add { id, caption, photos: ["/gallery/your-file.jpg", ...] } entries here
// Achievements: add { id, caption, photos: [...] } entries here
const IMAGES = {
  personal: [],
  projects: GALLERY_PROJECT_POSTS,
  achievements: [
    {
      id: 1,
      caption:
        "Youth Employment Program — Certificate of Appreciation (Star Performer) · TCS Empowers (Tata Consultancy Services) · 2026",
      photos: ["/certs/star p.jpg"],
    },
  ],
};
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {IMAGES[tab].length === 0 ? (
            ...
            >
              {tab === "personal" &&
                "No personal photos yet. Add posts in Gallery.jsx (personal array)."}
              {tab === "projects" &&
                "No projects in the gallery yet. Add entries in src/data/galleryProjects.js."}
              {tab === "achievements" &&
                "No achievements yet. Add posts in Gallery.jsx (achievements array)."}
            </p>
          ) : (
            IMAGES[tab].map((post) => (
              <motion.div
